...
const RoomTypeManagement = () => {
  ...
  const handleSave = async () => {
    try {
      ...
      alert('Failed to save room type: ' + (error?.response?.data?.detail || error?.message));
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      alert('Failed to delete room type: ' + (error?.response?.data?.detail || error?.message));
    }
  };

  const handleToggleActive = async (type) => {
    try {
      await roomTypesAPI.update(type?.id, {
        ...type,
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <Button onClick={handleCreate} iconName="Plus" data-testid="add-room-type-btn">
          Add Room Type
        </Button>
      </div>
      ...
      {(isCreating || editingType) && (
        <div className="bg-muted rounded-lg border border-border p-6">
          ...
          <div className="flex gap-2 mt-6">
            <Button onClick={handleSave} iconName="Check" data-testid="save-room-type-btn">
