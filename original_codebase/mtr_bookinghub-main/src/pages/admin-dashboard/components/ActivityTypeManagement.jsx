...
const ActivityTypeManagement = () => {
  ...
  const fetchActivityTypes = async () => {
    try {
      setLoading(true);
      const { data, error } = await supabase?.from('activity_types')?.select('*')?.order('created_at', { ascending: false });
  ...
  const handleSave = async () => {
    try {
      ...
      if (isCreating) {
        ...
        const { error } = await supabase?.from('activity_types')?.update(payload)?.eq('id', editingType?.id);
      ...
      alert('Failed to save activity type: ' + error?.message);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      alert('Failed to delete activity type: ' + error?.message);
    }
  };

  const handleToggleActive = async (type) => {
    try {
      const { error } = await supabase?.from('activity_types')?.update({ is_active: !type?.is_active })?.eq('id', type?.id);
