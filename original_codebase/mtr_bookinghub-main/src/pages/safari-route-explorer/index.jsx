...
const SafariRouteExplorer = () => {
  ...
  useEffect(() => {
    if (safariRoutes?.length > 0) {
      const currentRouteData = safariRoutes?.find(route => route?.id === selectedRoute);
      if (currentRouteData) {
        fetchWaypoints(currentRouteData?.route_id);
      }
    }
  }, [selectedRoute, safariRoutes]);
  ...
  const fetchData = async () => {
    try {
      ...
      const [routesResponse, zonesResponse] = await Promise.all([
        supabase?.from('safari_routes')?.select('*')?.eq('is_active', true)?.order('created_at', { ascending: true }),
        supabase?.from('wildlife_zones')?.select('*')?.eq('is_active', true)?.order('created_at', { ascending: true })
      ]);

      if (routesResponse?.error) throw routesResponse?.error;
      if (zonesResponse?.error) throw zonesResponse?.error;

      const formattedRoutes = routesResponse?.data?.map(route => ({
        id: route?.route_type,
        route_id: route?.id,
        name: route?.name,
        icon: route?.icon,
      ...
      const formattedZones = zonesResponse?.data?.map(zone => ({
        id: zone?.id,
        name: zone?.name,
  ...
  const fetchWaypoints = async (routeId) => {
    try {
      ...
      const formattedWaypoints = data?.map(wp => ({
        name: wp?.name,
        time: wp?.time,
      ...
      setSafariRoutes(prev => prev?.map(route => 
        route?.route_id === routeId 
          ? { ...route, waypoints: formattedWaypoints }
          : route
      ));
    } catch (error) {
